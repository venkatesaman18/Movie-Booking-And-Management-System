...
VALUES (3001, 1101, 1001, NOW(), 1.00, 'Confirmed');
...
-- delete in mysql- project

-- Step 1: Delete refunds (deepest level)
DELETE FROM refunds
WHERE payment_id IN (
    ...
    WHERE booking_id IN (
        ...
        WHERE showtime_id IN (
            SELECT showtime_id FROM showtimes WHERE movie_id = 901
        )
    )
);
...
-- Step 5: Delete showtimes
DELETE FROM showtimes WHERE movie_id = 901;

-- Delete reviews (direct child of movies)
DELETE FROM reviews WHERE movie_id = 901;
